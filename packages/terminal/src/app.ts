...
export class TerminalApp {
  constructor(options: ITerminalAppOptions) {
    ...
    const write = (text: string) => this._tty.write(text);
    this._screen = options.fullScreen
      ? new FullScreen(write, this._tty.size.rows)
      : new Screen(write);
  }
  ...
  /**
   * Scroll the transcript in full screen mode.
   */
  ...
  private _render(): void {
    ...
    if (this._mode === 'approval' && this._approval) {
      ...
      lines.push(...inputBox.lines, this._statusLine());
    }
  ...
  private _checkResize(): void {
    ...
    if (
      ...
      this._width = width;
      this._rows = rows;
      this._screen.resize(rows, columns);
      this._redrawAll();
      this._render();
    }
  }
