...
/**
 * Search commands using exact term matching across command metadata.
 *
 * Multi-word queries are split into individual terms, and every term must
 * appear in at least one searchable field. Results are ranked by stronger
  ...
  return tool({
    ...
    inputSchema: z.object({
      query: z
        ...
        .nullable()
        .describe(
          'Optional search query to filter commands. Supports multi-word queries by requiring each word to appear in the command id, label, caption, or description. Leave empty to list all commands.'
        )
    }),
